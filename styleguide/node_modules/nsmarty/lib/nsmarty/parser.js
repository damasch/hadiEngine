...
exports.parse = function (template, params, tpl_path) {
	var parser = new Parser(template, params, tpl_path);
 ...
 *	@Parser
 ** ******************************************* **/
function Parser(template, params, tpl_path) {
	this.tokens   		= ['multi'];
	this.partials 		= [];
	this.buffer   		= template;
	this.params   		= params;
	this.tpl_path   	= tpl_path;
	global.tpl_path 	= this.tpl_path;
	this.state    		= 'static'; // 'static' or 'tag'
	this.otag 			= '{leave_it_as_it_is', '}';
	this.ctag			= '';
}
...
Parser.prototype = {
	...
	scanText: function () {
		...
		var tpl 		= new nSmarty(content);

		var content = tpl.fetch(this.params);
...
/** ******************************************* **
	@iVersion 	0.1 [internal version]
	@Date 		Monday, July 09, 2012
	@Modified 	Dorin Grigoras
	@Original	Max Miroshnikov [http://code.google.com/p/jsmart/]
	@Changes	- Adapted to Node.js
				- Changed {include file=""} function,  added template path
				- Code still need cleanup, few more changes [next versions]

 * Original (c)
 * Copyright 2011, Max Miroshnikov <miroshnikov at gmail dot com> 
 * nSmarty is licensed under the GNU Lesser General Public License
...
** ******************************************* **/
    ...
    */
    function findTag(re,s)
    {
        var openCount = 0;
        var offset = 0;
        var ldelim = nSmarty.prototype.left_delimiter;
        var rdelim = nSmarty.prototype.right_delimiter;
        var skipInWS = nSmarty.prototype.auto_literal;
    ...
    var buildInFunctions = 
        {
            ...
            foreach: 
            {
                ...
                parse: function(params, tree, content)
                {
                    ...
                    var findElse = findElseTag('foreach\\s[^}]+', '\/foreach', 'else', content);
                    if (findElse)
                    {
                        parse(content.slice(0,findElse.index),subTree);
                        parse(content.slice(findElse.index+findElse[0].length).replace(/^[\r\n]/,''), subTreeElse);
            ...
            ldelim:
            {
                ...
                parse: function(params, tree)
                {
                    parseText(nSmarty.prototype.left_delimiter, tree);
                }
            },
            ...
            rdelim:
            {
                ...
                parse: function(params, tree)
                {
                    parseText(nSmarty.prototype.right_delimiter, tree);
                }
            },
    ...
    function lookUp(s,e)
    {
        ...
        if (s.substr(0,nSmarty.prototype.left_delimiter.length)==nSmarty.prototype.left_delimiter)
        {
            var tag = findTag('',s);
            if (tag)
            {
    ...
    function getTemplate(name, tree, nocache)
    {
        ...
        {
            var tpl = nSmarty.prototype.getTemplate(name);
            if (typeof(tpl) != 'string')
            {
                throw new Error('No template for '+ name);
            }
            parse(applyFilters(nSmarty.prototype.filters_global.pre, stripComments(tpl.replace(/\r\n/g,'\n'))), tree);
            files[name] = tree;
        }
        else
        {
    ...
    nSmarty = function(tpl)
    {
        ...
        this.smarty = {
            'smarty': {
                ...
                current_dir: '/',
                template: '',
                ldelim: nSmarty.prototype.left_delimiter,
                rdelim: nSmarty.prototype.right_delimiter,
                version: '2.9'
            }
        };
        blocks = this.tree.blocks;
        parse( 
            applyFilters(nSmarty.prototype.filters_global.pre, stripComments((new String(tpl?tpl:'')).replace(/\r\n/g,'\n'))), 
            this.tree
        );
    };

    nSmarty.prototype.fetch = function(data)
    {
        blocks = this.tree.blocks;
        scripts = this.scripts;
        escape_html = this.escape_html;
        default_modifiers = nSmarty.prototype.default_modifiers_global.concat(this.default_modifiers);
        this.data = obMerge((typeof data == 'object') ? data : {}, this.smarty);
        varFilters = nSmarty.prototype.filters_global.variable.concat(this.filters.variable);
        var res = process(this.tree, this.data);
        if (nSmarty.prototype.debugging)
        {
            plugins.debug.process([],this.data);
        }
        return applyFilters(nSmarty.prototype.filters_global.post.concat(this.filters.post), res);
    };

    nSmarty.prototype.escape_html = false;

    /**
       @param type  valid values are 'function', 'block', 'modifier'
       @param callback  func(params,data)  or  block(params,content,data,repeat)
    */
    nSmarty.prototype.registerPlugin = function(type, name, callback)
    {
        if (type == 'modifier')
        {
    ...
    */
    nSmarty.prototype.registerFilter = function(type, callback)
    {
        (this.tree ? this.filters : nSmarty.prototype.filters_global)[type=='output'?'post':type].push(callback);
    }

    nSmarty.prototype.filters_global = {'pre':[],'variable':[],'post':[]};

    nSmarty.prototype.configLoad = function(confValues, section, data)
    {
    ...
    nSmarty.prototype.clearConfig = function(varName)
    {
        if (varName)
        {
     ...
     */
    nSmarty.prototype.addDefaultModifier = function(modifiers)
    {
    ...
    nSmarty.prototype.default_modifiers_global = [];
    ...
    */
    nSmarty.prototype.getTemplate = function(name,tpl_path) {
        // throw new Error('No template for ' + name);
		return fs.readFileSync(global.tpl_path + name, 'utf8');
	}
    ...
    /**
       ...
       @return file content
    */
    nSmarty.prototype.getFile = function(name)
    {
    ...
    */
    nSmarty.prototype.getJavascript = function(name)
    {
    ...
    */
    nSmarty.prototype.getConfig = function(name)
    {
    ...
    */
    nSmarty.prototype.auto_literal = true;

    nSmarty.prototype.left_delimiter = '{';
    nSmarty.prototype.right_delimiter = '}';

    /** enables the debugging console */
    nSmarty.prototype.debugging = false;


    nSmarty.prototype.PHPJS = function(fnm, modifier)
    {
    ...
    nSmarty.prototype.makeTimeStamp = function(s)
    {
        if (!s)
        {
        ...
        if (isNaN(s))
        {
            var tm = nSmarty.prototype.PHPJS('strtotime','date_format').strtotime(s);
            if (tm == -1 || tm === false) {
                return Math.floor( (new Date()).getTime()/1000 );
            }
            return tm;
    ...
    */
    nSmarty.prototype.registerPlugin(
        'function', 
        '__array', 
    ...
    nSmarty.prototype.print_r = function(v,indent)
    {
        if (v instanceof Object)
        {
            ...
            for (var nm in v)
            {
                ...
                {
                    s += indent + '&nbsp;&nbsp;<strong>' + nm + '</strong> : ' + nSmarty.prototype.print_r(v[nm],indent+'&nbsp;&nbsp;&nbsp;') + '<br>';
                }
            }
            return s;
        }
    ...
    nSmarty.prototype.registerPlugin(
        ...
        function(params, data)
        {
            ...
            for (var nm in data)
            {
                sVars += '<tr class=' + (++i%2?'odd':'even') + '><td><strong>' + nm + '</strong></td><td>' + nSmarty.prototype.print_r(data[nm],'') + '</td></tr>';
            }
            dbgWnd.document.write(" \
               <html xmlns='http://www.w3.org/1999/xhtml' xml:lang='en'> \
               <head> \
		            <title>nSmarty Debug Console</title> \
                  <style type='text/css'> \
                     table {width: 100%;} \
    ...
    nSmarty.prototype.registerPlugin(
        ...
        function(params, data)
        {
            var s = nSmarty.prototype.getFile(params.__get('file',null,0));
            if ('assign' in params)
            {
                assignVar(params.assign, s, data);
                return '';
    ...
    nSmarty.prototype.registerPlugin(
        ...
        function(params, data)
        {
            ...
            scripts[file] = true;
            var s = execute(nSmarty.prototype.getJavascript(file), {'$this':data});
            if ('assign' in params)
            {
                assignVar(params.assign, s, data);
                return '';
    ...
    nSmarty.prototype.registerPlugin(
        ...
        function(params, data)
        {
            ...
            if ('script' in params)
            {
                eval(nSmarty.prototype.getJavascript(params.script));
    ...
    nSmarty.prototype.registerPlugin(
        ...
        function(params, data)
        {
            nSmarty.prototype.configLoad(nSmarty.prototype.getConfig(params.__get('file',null,0)), params.__get('section','',1), data);
            return '';
        }
    );
    ...
    nSmarty.prototype.registerPlugin(
        ...
        function(params, data)
        {
            ...
            var cc = nSmarty.prototype.PHPJS('rawurlencode','mailto').rawurlencode(params.__get('cc','')).replace('%40','@');
            var bcc = nSmarty.prototype.PHPJS('rawurlencode','mailto').rawurlencode(params.__get('bcc','')).replace('%40','@');
            var followupto = nSmarty.prototype.PHPJS('rawurlencode','mailto').rawurlencode(params.__get('followupto','')).replace('%40','@');
            var subject = nSmarty.prototype.PHPJS('rawurlencode','mailto').rawurlencode( params.__get('subject','') );
            var newsgroups = nSmarty.prototype.PHPJS('rawurlencode','mailto').rawurlencode(params.__get('newsgroups',''));
            ...
            if (encode == 'javascript')
            {
                ...
                for (var i=0; i<s.length; ++i)
                {
                    sEncoded += '%' + nSmarty.prototype.PHPJS('bin2hex','mailto').bin2hex(s.substr(i,1));
                }
            ...
            else if (encode == 'javascript_charcode')
            {
                ...
                for (var i=0; i<s.length; ++i) 
                {
                    codes.push(nSmarty.prototype.PHPJS('ord','mailto').ord(s.substr(i,1)));
                } 
            ...
            else if (encode == 'hex')
            {
                ...
                for (var i=0; i<address.length; ++i)
                {
                    ...
                    {
                        aEncoded += '%' + nSmarty.prototype.PHPJS('bin2hex','mailto').bin2hex(address.substr(i,1));
                    }
                    else
                    {
                ...
                {
                    tEncoded += '&#x' + nSmarty.prototype.PHPJS('bin2hex','mailto').bin2hex(text.substr(i,1)) + ';';
                }
    ...
    nSmarty.prototype.registerPlugin(
        ...
        function(params, data)
        {
            ...
            if ('format' in params)
            {
                res = nSmarty.prototype.PHPJS('sprintf','math').sprintf(params.format,res);
            }
    ...
    */
    ...
    nSmarty.prototype.registerPlugin(
        ...
        {
            return nSmarty.prototype.PHPJS('strftime','date_format').strftime(fmt?fmt:'%b %e, %Y', nSmarty.prototype.makeTimeStamp(s?s:defaultDate));
        }
    );
    ...
    nSmarty.prototype.registerPlugin(
        ...
        function(s, esc_type, char_set, double_encode)
        {
            ...
            case 'htmlall':
                return nSmarty.prototype.PHPJS('htmlentities','escape').htmlentities(s, 3, char_set);
            case 'url':
                return nSmarty.prototype.PHPJS('rawurlencode','escape').rawurlencode(s);
            case 'urlpathinfo':
                return nSmarty.prototype.PHPJS('rawurlencode','escape').rawurlencode(s).replace(/%2F/g, '/');
            case 'quotes': 
            ...
            case 'hex':
                ...
                for (var i=0; i<s.length; ++i) 
                {
                    res += '%' + nSmarty.prototype.PHPJS('bin2hex','escape').bin2hex(s.substr(i,1));
                } 
                return res;
            case 'hexentity':
                var res = '';
                for (var i=0; i<s.length; ++i) {
                    res += '&#x' + nSmarty.prototype.PHPJS('bin2hex','escape').bin2hex(s.substr(i,1)).toUpperCase() + ';';
                } 
                return res;
            case 'decentity':
                var res = '';
                for (var i=0; i<s.length; ++i) {
                    res += '&#' + nSmarty.prototype.PHPJS('ord','escape').ord(s.substr(i,1)) + ';';
                } 
                return res;
            case 'mail': 
            ...
            case 'nonstd': 
                ...
                for (var i=0; i<s.length; ++i)
                {
                    var _ord = nSmarty.prototype.PHPJS('ord','escape').ord(s.substr(i,1));
                    if (_ord >= 126) {
                        res += '&#' + _ord + ';';
                    } else {
                        res += s.substr(i, 1);
    ...
    */
    ...
    nSmarty.prototype.registerPlugin(
        ...
        function(s, fmt)
        {
            return nSmarty.prototype.PHPJS('sprintf','string_format').sprintf(fmt,s);
        }
    );
