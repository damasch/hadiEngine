...
nsmarty.compileText = function(name, template, callback) {
  ...
  try {
    parsed = parser.parse(template, null, nsmarty.tpl_path);
...
nsmarty.renderText = function (template, params, partials) {
  ...
  for (name in partials) {
    if (partials.hasOwnProperty(name) && !partials[name].tokens) {
      partials[name] = parser.parse(partials[name], null, nsmarty.tpl_path);
    }
  }
  parsed = parser.parse(template, null, nsmarty.tpl_path);
...
function beginRenderWithStream(tokens, params, partials, stream) {
  var count = 0;
  console.log("beginRenderWithStream 0");
  process.nextTick(function () {
    try {
		console.log("beginRenderWithStream 1");
      renderer.render(tokens, params, partials, stream, function () {
		  console.log("beginRenderWithStream 3");
        stream.emit('end');
      });
    } catch (err) {
    	console.log("beginRenderWithStream 2");
